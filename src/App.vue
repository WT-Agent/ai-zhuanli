  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">专利类型</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">交底书模块</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">专利交底书撰写与专利代理通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 专利交底书与技术交底专家');
...
const metricsList = [
  {
    "key": "patentDisclosureStandardRigor",
    "label": "交底书规范严谨度"
  },
  {
    "key": "technicalNoveltyHighlight",
    "label": "技术新颖性突显度"
  },
  {
    "key": "embodimentDetailFeasibility",
    "label": "实施例细节可行性"
  },
  {
    "key": "claimsProtectionScope",
    "label": "权利要求保护范围分"
  },
  {
    "key": "patentExaminerReadability",
    "label": "审查员审查易读度"
  }
];
const guideRules = [
  {
    "title": "充分公开达到可实施标准",
    "advice": "实施方式部分必须写明具体参数、步骤或模块结构，使本领域技术人员能够据以实施",
    "avoid": "切忌过度保密关键技术导致“公开不充分”被驳回"
  },
  {
    "title": "技术效果与技术特征对应",
    "advice": "每强调一项“有益效果”（如“提升响应速度 30%”），必须在技术方案中找到对应的技术特征支撑",
    "avoid": "避免凭空吹嘘技术效果而没有结构特征对应"
  },
  {
    "title": "独立权利要求保护范围最大化",
    "advice": "独权中只写入实现发明目的的最少必要技术特征，避免将非必要的优选特征写入独权限制保护范围",
    "avoid": "避免独权特征过多导致侵权避让过于容易"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[ZHUANLI_SCORES\][\s\S]*?\[\/ZHUANLI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[ZHUANLI_SCORES\](.*?)\[\/ZHUANLI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('zhuanli_history_records');
...
const saveHistory = () => {
  localStorage.setItem('zhuanli_history_records', JSON.stringify(historyList.value));
};
